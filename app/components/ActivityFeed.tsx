...
const ActivityFeed: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">실시간 활동</h1>
        <p className="text-gray-500">가족 구성원들의 최근 활동을 확인하세요</p>
      </div>
        ...
        {activities.length === 0 ? (
          ...
          <div className="space-y-4">
            {activities.map((activity) => (
              <div
                key={activity.id}
                className={`p-4 rounded-lg border-l-4 ${activity.type === 'add' ? 'border-green-500' : activity.type === 'update' ? 'border-blue-500' : activity.type === 'finish' ? 'border-purple-500' : activity.type === 'comment' ? 'border-orange-500' : 'border-indigo-500'} ${getActivityBackground(activity.type)}`}
              >
